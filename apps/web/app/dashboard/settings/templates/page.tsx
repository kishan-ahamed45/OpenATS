      ...
      {open && (
        <div className="absolute right-0 top-8 z-50 w-44 bg-white border border-slate-200 rounded-lg shadow-lg py-1 text-sm">
          <button onClick={() => { setOpen(false); onEdit() }}
            ...
            <HugeiconsIcon icon={PencilEdit01Icon} className="size-4 text-slate-400" /> Edit
          </button>
          <button onClick={() => { setOpen(false); onDuplicate() }}
            className="w-full flex items-center gap-3 px-4 py-2.5 text-slate-700 hover:bg-slate-50">
            <HugeiconsIcon icon={Copy01Icon} className="size-4 text-slate-400" /> Duplicate
          </button>
...
export default function TemplatesPage() {
  ...
  const [pickedType, setPickedType] = React.useState<TemplateType | null>(null)

  // Delete confirm
  const [deleteId, setDeleteId] = React.useState<number | null>(null)
  const deleteName = templates.find(t => t.id === deleteId)?.name

  // ── Load from store on mount (and on focus for cross-tab freshness)
  const refresh = React.useCallback(() => setTemplates(getTemplates()), [])
  React.useEffect(() => {
    refresh()
  ...
  const filtered = templates.filter(t => {
    ...
    return t.name.toLowerCase().includes(q) && (filterType === "all" || t.type === filterType)
  })
  ...
  return (
    <div className="flex flex-1 flex-col bg-white">
      ...
      <div className="px-8 py-4 flex items-center justify-between">
        <h1 className="text-[28px] font-medium text-slate-900 leading-none">Templates</h1>
        <button
          onClick={() => { setPickedType(null); setTypePickerOpen(true) }}
      ...
      <div className="border-y border-slate-200 px-8 py-3.5 flex items-center gap-4">
        ...
        {(search || filterType !== "all") && (
          <Button variant="ghost" onClick={() => { setSearch(""); setFilterType("all") }}
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b border-slate-200 bg-white hover:bg-transparent">
                <TableHead className="h-13 px-8 font-semibold text-slate-900 text-sm">Template Name</TableHead>
                <TableHead className="h-13 px-8 font-semibold text-slate-900 text-sm">Type</TableHead>
                <TableHead className="h-13 px-8 font-semibold text-slate-900 text-sm">Created By</TableHead>
                <TableHead className="h-13 px-8 font-semibold text-slate-900 text-sm">Last Edited</TableHead>
            ...
            <TableBody>
              {filtered.length === 0 ? (
                ...
                <TableRow key={t.id} className="border-b border-slate-200 last:border-0 font-medium hover:bg-slate-50/50">
                  <TableCell className="h-14 px-8 py-0">
                    <Link href={`/dashboard/settings/templates/${t.id}/edit`}
                      className="text-[#355872] font-medium hover:underline decoration-1 underline-offset-4">
                      {t.name}
                    </Link>
                  </TableCell>
                  <TableCell className="h-14 px-8 py-0">
                    <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${TYPE_META[t.type].badge}`}>
                      {TYPE_META[t.type].label}
                    </span>
                  </TableCell>
                  <TableCell className="h-14 px-8 py-0 text-[#355872] font-normal">{t.createdBy}</TableCell>
                  <TableCell className="h-14 px-8 py-0 text-[#355872] font-normal">{t.editedAt}</TableCell>
                  <TableCell className="h-14 px-4 py-0">
                    <RowMenu
                      onEdit={() => router.push(`/dashboard/settings/templates/${t.id}/edit`)}
      ...
      <Dialog open={typePickerOpen} onOpenChange={setTypePickerOpen}>
        <DialogContent className="!top-[20%] !translate-y-0 sm:max-w-[500px] max-w-[500px] rounded-xl border-slate-200 shadow-lg p-7 duration-0 data-open:zoom-in-100 data-closed:zoom-out-100">
          <DialogHeader>
            <DialogTitle className="text-[18px] font-semibold text-slate-900">What type of template is this?</DialogTitle>
            <p className="text-[13px] text-slate-500 mt-1">The type sets which variables are available in the builder.</p>
          </DialogHeader>

          <div className="grid grid-cols-2 gap-3 mt-5">
            {(["offer", "rejection", "assessment", "general"] as TemplateType[]).map(t => (
              <button key={t} onClick={() => setPickedType(t)}
                className={`flex flex-col items-start gap-2.5 p-4 rounded-xl border-2 text-left transition-all ${
                  pickedType === t ? "border-[#355872] bg-[#355872]/5" : "border-slate-200 hover:border-slate-300 bg-white"
                }`}>
                <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${TYPE_META[t].badge}`}>
                  {TYPE_META[t].label}
                </span>
                <span className="text-[12px] text-slate-500 leading-snug">
                  {t === "offer"      && "Offer letters with salary & start date"}
                  {t === "rejection"  && "Notify candidates who weren't selected"}
          ...
          <DialogFooter className="mt-6 gap-2">
            <Button variant="outline" onClick={() => setTypePickerOpen(false)}
              className="h-10 px-6 border-slate-200 text-slate-600 hover:bg-slate-50 font-medium shadow-none rounded-lg text-sm">
              Cancel
            </Button>
            <Button
              disabled={!pickedType}
              onClick={() => { if (pickedType) { setTypePickerOpen(false); router.push(`/dashboard/settings/templates/new?type=${pickedType}`) } }}
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={o => !o && setDeleteId(null)}>
        <AlertDialogContent className="max-w-sm rounded-xl border-slate-200 shadow-lg">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-[17px] font-semibold text-slate-900">Delete Template?</AlertDialogTitle>
            <AlertDialogDescription className="text-[13px] text-slate-500 leading-relaxed">
              <strong className="text-slate-700">{deleteName}</strong> will be permanently removed. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
