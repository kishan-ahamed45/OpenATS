    ...
    <div className="flex flex-1 flex-col bg-white">
      ...
      <div className="px-8 pt-10 pb-0 max-w-5xl w-full">
        ...
        <div className="space-y-2.5 mb-6">
          ...
          <Link href={`/careers/${id}`} target="_blank" className="flex items-center gap-2 text-[#355872] text-[15px] font-medium hover:underline cursor-pointer group w-fit">
              ...
              {stages.map((stage, index) => {
                function StageDraggable() {
                  const { ref, isDragging, isOver } = useDragSort({
                    ...
                    index,
                    type: 'HIRING_STAGE',
                    onMove: (from, to) => setStages(prev => moveItem(prev, from, to)),
                  })
                  return (
                    <div
                      ref={ref as React.Ref<HTMLDivElement>}
                      className={`flex items-center justify-between p-4 border rounded-lg transition-all group bg-white ${
                        isDragging ? 'opacity-40 border-slate-300' : isOver ? 'border-[#355872]/40 bg-[#355872]/5' : 'border-slate-200/70 hover:border-slate-300'
                      }`}
                    >
                      <div className="flex items-center gap-4">
                        <HugeiconsIcon icon={DragDropVerticalIcon} className="size-5 text-slate-300 group-hover:text-slate-400 cursor-grab active:cursor-grabbing" />
                        <div className={`size-2 rounded-full ${stage.color}`} />
                        <span className="text-slate-700 font-medium text-[15px]">{stage.name}</span>
                      </div>
                      <div className="flex items-center gap-4">
                        <button onClick={() => openConfigure({ id: stage.id, name: stage.name })} className="text-[#355872]/60 hover:text-[#355872] transition-colors" title="Configure Stage">
                        ...
                        <button onClick={() => setStages(prev => prev.filter(s => s.id !== stage.id))} className="text-red-400/80 hover:text-red-500 transition-colors">
                ...
                {questions.map((q, index) => {
                  function QuestionDraggable() {
                    const { ref, isDragging, isOver } = useDragSort({
                      ...
                      onMove: (from, to) => setQuestions(prev => moveItem(prev, from, to)),
                    })
                    return (
                      <div
                        ref={ref as React.Ref<HTMLDivElement>}
                        className={`group relative flex items-center justify-between p-4 border rounded-lg bg-white transition-all ${
                          isDragging ? 'opacity-40 border-slate-300' : isOver ? 'border-[#355872]/40 bg-[#355872]/5' : 'border-slate-200 hover:border-slate-300'
                        }`}
                      >
                        <div className="flex items-center gap-4">
